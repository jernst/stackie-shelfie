...
module peg_A( hu, through_h = $dz_unit/2 ) {
    ...
    difference() {
        ...
        rotate_extrude( angle=360 ) {
            translate( [ $r1, -$eps, 0 ] ) {
                union() {
                    square( [ $r2 + ( $r2 - $r1 ), through_h + 4*$eps - ($r2 - $r1) ] );
                    translate( [ $r2 - $r1, through_h + 2*$eps - ($r2 -$r1), 0 ] ) {
                        circle( r = $r2 - $r1 );
                    }
                    translate( [ $r2 - $r1, -$eps, 0 ] ) {
                        square( [ $r2 + $r2 - $r1, through_h + 2 * $eps ] );
                    }
                }
            }
        };

        translate( [ $dx_unit/2, $dy_unit/2, h - $dz_unit * 1.5 + through_h ] ) {
